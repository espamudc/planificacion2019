...
namespace GestionPOA.Controllers
{
    public class ObjeEspecificosDepartamentoesController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Create(int tipoDependenciaID, int departamentoID,int objetivoEspecificoID)
        {
            var listaOD = db.ObjeEspecificosDepartamento.Where(od => od.eliminado == false)
                                                         ...
                                                         .Where(od => od.TipoDependenciaID == tipoDependenciaID)
            ...
            if (listaOD.Count > 0)
            {
                return Json(new { success = false, mensaje = "El objetivo especifico seleccionado ya fue asignado a ese departamento" }, JsonRequestBehavior.AllowGet);
            }
            else {
                ...
                ObjeEspecificosdepartamento.TipoDependenciaID = tipoDependenciaID;
